...
## Шаги тестового задания

### 1 шаг

Страница сверстана по макету, рабочий процесс покупки по карточке товара выполнен с переходом на экран заказа, где с помощью polling отслеживается статус заказа
...
```text
Пока только локально, но могу развернуть на vds
```
